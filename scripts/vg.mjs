...
if (command === 'help') {
  console.log([
    ...
    '  /pause --task <id> [msg]   Freeze a specific task immediately + enter meetup mode.',
    '                              Optionally pre-inject a message before resume.',
    '  /msg --task <id> <text>    Inject a message into a running or frozen task\'s inbox.',
    '  /done                      Resume all frozen tasks (task-level or global).',
    '  /task <description>        Quickly add a new task to the queue.',
    '',
    'Human Alignment (leader-initiated):',
    '  When a leader writes status="waiting_for_human" + question to progress.json,',
    '  the task is auto-paused and you will see a 🤔 prompt asking for your input.',
    '  Type your response in the terminal, then /done to resume.',
    '',
